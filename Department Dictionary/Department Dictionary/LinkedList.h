...
template<class ItemType>
int LinkedList<ItemType>::getPosition(const ItemType in) const
{
    int out = 1;
    Node<ItemType>* curPtr = headPtr;
...
void LinkedList<ItemType>::display() 
{
    ...
    if (itemCount != 0)
    {
        ...
        while (curPtr != nullptr)
        {
            cout << curPtr->getItem() << " Pay: $" << count * 1000.00 << endl;
            count--;
            curPtr = curPtr->getNext();
        }
    }
...
template<class ItemType>
void LinkedList<ItemType>::displayUnemployed()
{
    Node<ItemType>* curPtr = headPtr;
