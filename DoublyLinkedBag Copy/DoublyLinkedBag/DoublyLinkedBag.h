...
public:
    ...
    void insertSorted(const ItemType  newEntry[], int size);
}; // end DoublyLinkedBag
...
template<class ItemType>
void DoublyLinkedBag<ItemType>::insertSorted(const ItemType newEntry[], int size)
{
    DoublyNode<ItemType>* curPtr = headPtr;

        for (int i = 0; i < size; i++)
        {
            while (newEntry[i] > curPtr->getItem() && curPtr->getNext() != nullptr)
                curPtr = curPtr->getNext();
            if (headPtr == curPtr && newEntry[i] <= curPtr->getItem())
            {
                DoublyNode<ItemType>* tempNode = new DoublyNode<ItemType>();
                tempNode->setItem(newEntry[i]);
                tempNode->setNext(curPtr);
                curPtr->setPrev(tempNode);
                headPtr = tempNode;
                itemCount++;
            }

            else if (newEntry[i] <= curPtr->getItem())
            {
                DoublyNode<ItemType>* tempNode = new DoublyNode<ItemType>();
                tempNode->setItem(newEntry[i]);
                curPtr->getPrev()->setNext(tempNode);
                tempNode->setPrev(curPtr->getPrev());
                tempNode->setNext(curPtr);
                curPtr->setPrev(tempNode);
                itemCount++;
            }
            else // newEntry[i] > lastNode 
            {
                DoublyNode<ItemType>* tempNode = new DoublyNode<ItemType>();
                curPtr->setNext(tempNode);
                tempNode->setItem(newEntry[i]);
                tempNode->setPrev(curPtr);
                itemCount++;
                curPtr = curPtr->getNext();
            }
        }
